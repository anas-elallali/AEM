...
export class ProfileService {
  ...
  protected convertDateFromClient(profile: IProfile): IProfile {
    const copy: IProfile = Object.assign({}, profile, {
      birthDate: profile.birthDate && profile.birthDate.isValid() ? profile.birthDate.toJSON() : undefined,
    });
    return copy;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((profile: IProfile) => {
        profile.birthDate = profile.birthDate ? moment(profile.birthDate) : undefined;
      });
    }
    return res;
  }
